...
function getPolynom(...coefficients) {
  ...
  return function createPolynomFunction(x) {
    const result = coefficients.reduce((acc, value, index) => {
      return acc + value * x ** (coefficients.length - index - 1);
    }, 0);
    return result;
  };
}
...
function memoize(func) {
  let result = null;
  let isResultCached = false;

  return function memoizeResult(argument) {
    if (isResultCached) {
      return result;
    }
...
function logger(func, logFunc) {
  function loggerFunction(...args) {
    const argArray = args.reduce((acc, value) => {
      acc.push(JSON.stringify(value));
      return acc;
    }, []);

    logFunc(`${func.name}(${argArray.join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argArray.join(',')}) ends`);
...
function partialUsingArguments(fn, ...args1) {
  function argumentsReapplier(...args2) {
    return fn(...args1, ...args2);
  }
  return argumentsReapplier;
}
